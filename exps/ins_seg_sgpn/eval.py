...
pw_sim_thres = np.loadtxt(os.path.join(VALID_DIR, 'per_category_pointwise_similarity_threshold.txt'))
avg_group_size = np.loadtxt(os.path.join(VALID_DIR, 'per_category_average_group_size.txt'))
...
def gen_visu(base_idx, pts, mask, valid, conf, sem, record, num_pts_to_visu=1000):
    ...
    for i in range(n_shape):
        ...
        render_pts(out_fn, cur_pts[cur_idx_to_visu])
        ...
        for j in range(n_ins):
            ...
            if cur_valid[cur_idx]:
                ...
                render_pts_with_label(out_fn, cur_pts[cur_idx_to_visu], cur_mask[cur_idx, cur_idx_to_visu].astype(np.int32))
...
def eval():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            seg_pred = net_output['semseg_logits']                                  # B x N x C
        ...
        for item in test_h5_fn_list:
            ...
            for i in bar(range(num_batch)):
                ...
                seg_pred_val, simmat_pred_val, conf_pred_val = sess.run([seg_pred, simmat_pred, conf_pred], feed_dict=feed_dict)

                seg_pred_val = seg_pred_val[:end_idx-start_idx, ...]        # B x N x C
                seg_pred_res = np.argmax(seg_pred_val, axis=-1)
                ...
                for j in range(end_idx - start_idx):
                    ...
                    cur_seg = np.argmax(seg_pred_val[j, ...], axis=-1)

                    group_ids, _, group_seg, group_conf = GroupMerging(cur_simmat, cur_conf, cur_seg, pw_sim_thres)
                    ...
                    for gid in group_seg.keys():
                        if np.sum(group_ids == gid) >= min_group_size[group_seg[gid]]:
                            ...
                            tmp = (group_ids == gid)
                            out_label[sid, gid] = int(stats.mode(seg_pred_res[j, tmp])[0])
                ...
                if FLAGS.visu_dir is not None and cur_visu_batch < FLAGS.visu_batch:
                    gen_visu(start_idx, batch_pts[:end_idx-start_idx, ...], out_mask[start_idx: end_idx, ...], out_valid[start_idx: end_idx, :], \
                            out_conf[start_idx: end_idx, :], out_label[start_idx: end_idx, :], batch_record)
                    cur_visu_batch += 1
