...
parser.add_argument('--split', default='test', type=str, help='split [default: test]')
...
def save_h5(fn, pts, gt_label, gt_mask, gt_valid, gt_other_mask):
    ...
    fout.create_dataset('gt_mask_label', data=gt_label, compression='gzip', compression_opts=4, dtype='uint8')
    fout.create_dataset('gt_mask', data=gt_mask, compression='gzip', compression_opts=4, dtype='bool')
    fout.create_dataset('gt_mask_valid', data=gt_valid, compression='gzip', compression_opts=4, dtype='bool')
    fout.create_dataset('gt_mask_other', data=gt_other_mask, compression='gzip', compression_opts=4, dtype='bool')
    fout.close()
...
def reformat_data(in_h5_fn, out_h5_fn):
    ...
    gt_point_label = np.zeros((n_shape, NUM_POINT), dtype=np.uint8)
    gt_mask_label = np.zeros((n_shape, NUM_INS), dtype=np.uint8)
    gt_mask = np.zeros((n_shape, NUM_INS, NUM_POINT), dtype=np.bool)
    gt_valid = np.zeros((n_shape, NUM_INS), dtype=np.bool)
    gt_other_mask = np.zeros((n_shape, NUM_POINT), dtype=np.bool)
        ...
        for item in cur_record['ins_seg']:
            if item['part_name'] in part_name_list:
                ...
                if np.sum(selected) > 0:
                    ...
                    gt_mask_label[i, cur_tot] = sem_id
                    gt_point_label[i, selected] = sem_id + 1
                    gt_mask[i, cur_tot, selected] = True
                    gt_valid[i, cur_tot] = True
                    cur_tot += 1
        gt_other_mask[i, :] = (gt_point_label[i, :] == 0)

    save_h5(out_h5_fn, pts, gt_mask_label, gt_mask, gt_valid, gt_other_mask)

# main
data_in_dir = '../../data/ins_seg_h5/%s/' % args.category
data_out_dir = '../../data/ins_seg_h5_gt'
