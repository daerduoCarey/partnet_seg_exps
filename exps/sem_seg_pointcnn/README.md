...
Adapted from https://github.com/yangyanli/PointCNN. Please see the original github repo for LICENSE.

We try to keep the default setting as much as possible. We adapt the settings and hyper-parameters from the original ShapeNet Part experiment in the original PointCNN paper.

We make very few modifications:

* Input point size is changed from 2,048 to 10,000 points, and all input shapes are guaranteed to have 10,000 points;
* To fit the training for Titan XP GPUs with 12G memory, we have to reduce the batch size from 16 to 4;
* During the testing time, instead of outputing PLY files with the predicted labels, we output the prediction in HDF5 file, which makes the follow-up mIoU computation faster;
* Following the default evaluation benchmark, we train separate networks for different object categories and different levels of segmentation.
